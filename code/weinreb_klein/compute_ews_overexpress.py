...
Compute EWS over data with genes overexpressed
...
list_protocols = ["overexpress_{}".format(ko) for ko in [0, 5, 10, 20, 30, 40, 50]]
...
for protocol in list_protocols:
    ...
    # Export data
    df_dl.to_csv("output/ews/df_dl_overexpress.csv", index=False)
    df_ews.to_csv("output/ews/df_ews_overexpress.csv", index=False)
