...
Functions for simulating models for training data
...
def iterate_ns(s, mu, theta, dict_coeffs_x, dict_coeffs_y, supercrit=True):
    ...
    s_next = (
        np.matmul((1 + mu) * R, s)
        + cubic_coeff * np.matmul((x**2 + y**2) * R, s)
        + v_hot
    )
...
def iterate_fold(x, mu, dict_coeffs, orient=1):
    """
    ...
    dict_coeffs : dictionary.
        ...
        Values are weights/parameters for each higher-order term.
    orient : orientation of fold bifurcation
        1 means stable branch is positive
        -1 means stable branch is negative
    ...
    if orient == 1:
        ...
        # Orientation with negative branch stable
        # Get sum of higher-order terms
        sum_hot = 0
        for order in dict_coeffs.keys():
            sum_hot += dict_coeffs[order] * (x + np.sqrt(max(-mu, 0))) ** order
...
def simulate_pd(
    ...
    dict_coeffs = {
        order: np.random.normal(0, 1) for order in np.arange(4, max_order + 1)
    }
...
def simulate_ns(
    ...
    for i in range(int(tburn)):
        s0 = (
            iterate_ns(s0, bl, theta, dict_coeffs_x, dict_coeffs_y, supercrit)
            + dW_burn[i]
        )
        # If blows up
    ...
    for i in range(len(t) - 1):
        s[i + 1] = (
            iterate_ns(s[i], b.iloc[i], theta, dict_coeffs_x, dict_coeffs_y, supercrit)
            + dW[i]
        )
...
def simulate_fold(
    ...
    x0 = np.sqrt(-bl) if orient == 1 else -np.sqrt(-bl)
    # Time values
    t = np.arange(0, tmax, 1)
    # Linearly increasing bifurcation parameter
    b = pd.Series(np.linspace(bl, bh, len(t)), index=t)
    ...
    dict_coeffs = {
        order: np.random.normal(0, 1) for order in np.arange(3, max_order + 1)
    }
    ...
    # Run burn-in period on x0
    for i in range(int(tburn)):
        x0 = iterate_fold(x0, bl, dict_coeffs, orient=orient) + dW_burn[i]
        # If blows up
        if abs(x0) > 1e6:
            print("Model diverged during burn in period")
            x = np.ones(len(t)) * np.nan
    ...
    x[0] = x0
    for i in range(len(t) - 1):
        x[i + 1] = iterate_fold(x[i], b.iloc[i], dict_coeffs, orient=orient) + dW[i]
        # Stop if transitioned
        if orient == 1:
    ...
    # Put in df
    df_traj = pd.DataFrame({"time": t, "x_raw": x, "b": b})

    # Get residuals from equilibrium
    if orient == 1:
        equi = np.sqrt((-b).apply(lambda x: max(x, 0)))
    else:
        equi = -np.sqrt((-b).apply(lambda x: max(x, 0)))

    if return_dev:
        df_traj["x"] = df_traj["x_raw"] - equi
    else:
...
def simulate_tc(
    bl=-1, bh=0, tmax=500, tburn=100, sigma=0.01, max_order=10, dev_thresh=0.4
):
    """
