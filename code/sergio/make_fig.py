...
Make fig with rows
- Sergio PCA 1st component
- Residuals from detrending
- Variance (averaged over top 10 PCA)
- DL predictions (averaged over top 10 PCA)

Columns:
- Cell trajectory up to pitchfork bifurcation (forced)
- AR1 trajectory (null)
...
df_ews_forced = pd.read_csv("output/ews/df_ews.csv")

df_dl_forced = pd.read_csv("output/ews/df_dl.csv")
df_dl_forced = df_dl_forced.groupby("time").mean(numeric_only=True).reset_index()
df_dl_forced = df_dl_forced.query("pseudotime_index>=40")
...
df_dl_null = df_dl_null.query("pseudotime_index>=40")
...
df_dl_null = df_dl_null.query("pseudotime<=@transition_pseudo")
...
df_plot["pseudotime_index"] = df_plot["pseudotime_index"] - first_idx
# init_val = df_plot['pseudotime_index'].iloc[0]
# df_plot['pseudotime_index'] = np.arange(init_val,init_val+len(df_plot))

transition = transition - first_idx
fig.add_trace(
    go.Scatter(
        x=df_plot[xvar],
        y=df_plot["value"],
...
fig.update_yaxes(range=[-0.05, 1.05], row=2)
...
# Export image
fig.write_image("figures/fig_sergio.png", scale=2)
fig.write_image("figures/fig_sergio.pdf", scale=2)
